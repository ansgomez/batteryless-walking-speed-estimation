...
/**
 * Initialize the camera specific peripherals: DMA and PWM timers.
 * @note DMA and timers are configured only. Set up ADC and GPIOs separately
 * using Camera_setupADC() and Camera_initGPIO(), respectively!
 */
void Camera_init(void) {
	...
	// congifure DMA: repeated single mode, increasing destination address, byte transfer
	// interrupt clear and enable
	DMA1CTL = DMADT_4 | DMADSTINCR_3 | DMADSTBYTE | DMASRCBYTE | DMAIE;

	// configure TA0 in up mode
	TIMER_A_initUpModeParam timer_a0_init_struct = {
		.clockSource = TIMER_A_CLOCKSOURCE_SMCLK,
		.clockSourceDivider = TIMER_A_CLOCKSOURCE_DIVIDER_1,
		.timerPeriod = CAMERA_ROW_TIMER_CYCLES - 1,
	...
	TIMER_A_initCompareModeParam timer_a0_init_compare2_struct = {
		...
		.compareValue = CAMERA_PULSE_TIMER_CYCLES,
	};
	...
	TIMER_A_initUpModeParam timer_a1_init_struct = {
		.clockSource = TIMER_A_CLOCKSOURCE_SMCLK,
		.clockSourceDivider = TIMER_A_CLOCKSOURCE_DIVIDER_1,
		.timerPeriod = CAMERA_PIXEL_TIMER_CYCLES - 1,
		.timerInterruptEnable_TAIE = TIMER_A_TAIE_INTERRUPT_DISABLE,
		.captureCompareInterruptEnable_CCR0_CCIE =
				TIMER_A_CCIE_CCR0_INTERRUPT_DISABLE,
		.timerClear = TIMER_A_SKIP_CLEAR,
	...
	TIMER_A_initCompareModeParam timer_a1_init_compare1_struct = {
		...
		.compareValue = CAMERA_PIXEL_TIMER_CYCLES
	...
	TIMER_B_initCompareModeParam timer_b0_init_compare2_struct = {
		...
		.compareValue = CAMERA_ROW_TIMER_CYCLES - CAMERA_PULSE_TIMER_CYCLES,
	};
...
//	REFCTL0 = REFVSEL_0 | REFTCOFF | REFON;
	...
	// configure sampling resolution to 10 bits
 ...
 */
void Camera_startAquire(uint8_t* image_base) {
	// set busy flag
	camera_busy = true;

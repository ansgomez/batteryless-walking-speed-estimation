...
uint8_t SPI_transceive(uint32_t base_address, uint8_t data) {
    ...
    EUSCI_A_CMSIS(base_address)->IFG &= ~(EUSCI_A_SPI_TRANSMIT_INTERRUPT);
	...
	while (!(EUSCI_A_CMSIS(base_address)->IFG & EUSCI_A_SPI_TRANSMIT_INTERRUPT) ) {
		continue;
	}
...
void EXTFRAM_powerOn(void) {
	...
	uint32_t n = 300;
	while (n) {
		__delay_cycles(CPU_FREQUENCY/1000000);
		n--;
	}
}
...
uint16_t EXTFRAM_write(uint32_t address, const void* data, uint16_t count) {
	...
	while(i < count) {
	    ...
	    EUSCI_A_CMSIS(FRAM_SPI_PERIPHERAL)->IFG &= ~(EUSCI_A_SPI_TRANSMIT_INTERRUPT);
	    EUSCI_A_CMSIS(FRAM_SPI_PERIPHERAL)->TXBUF = data_ptr[i++];
		// wait until trasmission complete
		while (!(EUSCI_A_CMSIS(FRAM_SPI_PERIPHERAL)->IFG & EUSCI_A_SPI_TRANSMIT_INTERRUPT) ) {
			continue;
		}
	}

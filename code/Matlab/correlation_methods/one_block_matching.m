function [ vx, vy ] = one_block_matching( ImageA, ImageB )
...
%   estimates the displacement between image ImageA and image ImageB
    ...
    if ( numrows ~= size(ImageB,1) || numcols ~= size(ImageB,2) )
     ...
     %% Algorithm
     I(1, 1:numrows*numcols) = double(ImageA(:));
     I(2, 1:numrows*numcols) = double(ImageB(:));
     ...
     for i_search_x = (searcharea_x_neg):searcharea_x_pos
         ...
         for i_search_y = (searcharea_y_neg):searcharea_y_pos
             ...
             cost = 0;
             for i_x = 0:(eff_numpixels_x-1)
                 for i_y = 0:(eff_numpixels_y-1)

                     p_I1 = (p_I1start_x + i_x) * numpixels_y + p_I1start_y + i_y;
                     p_I2 = (p_I2start_x + i_x) * numpixels_y + p_I2start_y + i_y;

                     %[p_I1_x p_I1_y p_I2_x p_I2_y]
                     % Sum of Absolute Differences (SAD)
                     cost = cost + (abs(I(1, p_I1+1) - I(2, p_I2+1)));
                     if (cost >= 4294967296)

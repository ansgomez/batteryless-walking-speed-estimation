    ...
    if ( numrows ~= size(ImageB,1) || numcols ~= size(ImageB,2) )
     ...
     for i_block_x = 0:(numblocks_x-1)
         for i_block_y = 0:(numblocks_y-1)
             ...
             p_I1start_x = i_block_x * blocksize_x;
             p_I1start_y = i_block_y * blocksize_y;
                 ...
                 for i_search_y = (searcharea_y_neg):searcharea_y_pos
                     ...
                     for i_x = 1:blocksize_x
                         for i_y = 1:blocksize_y
                             ...
                             if ( p_I1_x > numpixels_x ) p_I1_x = 1; end
                             if ( p_I1_y < 1 ) p_I1_y = 1; end
                             if ( p_I1_y > numpixels_y ) p_I1_y = 1; end
                             if ( p_I2_x < 1 ) p_I2_x = 1; end
                             if ( p_I2_x > numpixels_x ) p_I2_x = 1; end
                             if ( p_I2_y < 1 ) p_I2_y = 1; end
                             if ( p_I2_y > numpixels_y ) p_I2_y = 1; end
                             
                             %[p_I1_x p_I1_y p_I2_x p_I2_y]
                             % Sum of Absolute Differences (SAD)
                             cost = cost + floor(abs(I(1, p_I1_y, p_I1_x) - I(2, p_I2_y, p_I2_x)));
                             if (cost >= 65535)
                                 cost
                             end
                         end

function [OF_x, OF_y] = block_matching( ImageA, ImageB, blocksize, searcharea )
%BLOCK_MATCHING estimates the optical flow between two images.
%
%   [OF_x, OF_y] = BLOCK_MATCHING(ImageA, ImageB, blocksize, searcharea)
%   estimates the optical flow between image ImageA and image ImageB.
%   The function returns an optical flow field.
%
...
%   Author: Thomas Schalch, 2016, ETHZ
    ...
    if ( numrows ~= size(ImageB,1) || numcols ~= size(ImageB,2) )
     ...
     OF_x = zeros(numblocks_y, numblocks_x);
     OF_y = zeros(numblocks_y, numblocks_x);
             ...
             for i_search_x = (searcharea_x_neg):searcharea_x_pos
                 ...
                 for i_search_y = (searcharea_y_neg):searcharea_y_pos
                     ...
                     if ( cost < mincost )
                         ...
                         OF_x(i_block_y+1, i_block_x+1) = i_search_x;
                         OF_y(i_block_y+1, i_block_x+1) = i_search_y;
                     end
